...
<div
	...
	class='bg-gray-800/75 flex flex-row flex-wrap px-2
 	fixed bottom-5 right-5 backdrop-blur-sm
	rounded-xl z-20'
	style={`width:${width - 42}px;`}
>
	<div class='flex flex-row flex-wrap gap-2 items-center px-1'>
		<h6 class='text-white bg-gray-700/80 rounded-lg my-2 py-1 px-2'>선택됨</h6>
		<div class='flex flex-row gap-1'>
			<h6 class='text-green-200 italic font-semibold'>
			...
			<h6 class='text-green-200 italic font-semibold'>{sectionId}구역-{lockerNum}</h6>
		</div>
	</div>
	<div class='flex flex-row flex-end flex-grow justify-end gap-2 items-center mr-1'>
		<Button
			on:click={() => click('secondary')}
			class='{secondaryClass} !py-2 !px-3 bg-gray-200 text-gray-600 text-center [&[disabled]]:opacity-50'
		>
			선택 해제
		</Button>
		<Button
